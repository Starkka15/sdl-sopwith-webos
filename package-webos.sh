#!/bin/bash
# Package SDL Sopwith for webOS

set -e

PKGDIR=webos-pkg
APPID=com.sopwith.game
APPDIR=$PKGDIR/usr/palm/applications/$APPID
OUTFILE=${APPID}_2.9.0_armv7.ipk
...
# Copy binary
echo "Copying binary..."
cp src/sopwith $APPDIR/

# Copy game maps
echo "Copying game maps..."
cp -r maps $APPDIR/
...
# Copy icon
if [ -f "src/sopwith.png" ]; then
    cp src/sopwith.png $APPDIR/icon.png
else
    echo "Note: No icon.png found"
fi
